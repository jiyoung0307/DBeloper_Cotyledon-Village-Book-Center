...
public class DBeloper_Service {
	...
	/* 고객 로그인용 Service (수정 사항 : 기존에 이름만 받아왔는데 필요한 정보를 DTO 형태로 다 받아오도록 수정) */
	...
	public int updateOutputOrderHistory(String managerInput) {
		...
		if(memberNo == null) {
				if(numberOfBooks > numberOfBookStock) {
					...
					/* (출고 승인) OutputOrderHistory outputApproval N -> Y */ 
	...
	/* 도수 수량으로 ORDER BY 후 도서 조회용 Service */
	...
	/* 특정 회원이 작성하거나 특정 도서에 대한 리뷰를 조회하는 메소드 */
